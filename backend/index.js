...
// Initialize GPTScript and set the API Key
const g = new GPTScript({ apiKey: process.env.OPENAI_API_KEY});
...
app.get('/build-video', async (req, res) => {
  ...
  for (let i = 0; i < images.length; i++) {
    ...
    // Build the drawtext filter string for subtitles
    let drawtextFilter = '';
...
app.get('/samples', (req, res) => {
  const stories = fs
    ...
    .filter((dir) => {
      return (
        dir.match(/^[a-z0-9]{6,}$/) &&
        fs.existsSync(`./stories/${dir}/final.mp4`)
      );
    });
  res.json(stories);
});
